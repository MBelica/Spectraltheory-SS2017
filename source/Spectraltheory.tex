...
We say that $T$ is \textbf{bounded} if it is continuous from $\DO{T}$ into $H$, with respect to the topology induced by $H$. We recall for the definition of bounded operators that if $\DO{T} = H$ holds, boundedness is equivalent to continuity in $0$, boundedness of $T(U_{(X,\|\cdot\|)})$ in $Y$ and there $\exists c < \infty$ such that $\| T x \| \leq c \| x \|$, for proof see the \href{https://github.com/MBelica/Funktionalanalysis-WS2015}{functional analysis} course. ~\bigskip

\index{dense}
 From now on, if $\DO{T} \neq H$ we will assume that $\DO{T}$ is \textbf{dense} in $H$, i.e. $\overline{\DO{T}} = H$. If in this case $T$ would be bounded then $T$ has a unique continuous extension to all of $H$. As this simplifies many considerations some of the following theorems would be trivial, and hence, we won't focus on bounded operators during this lecture.
...
\begin{example}
	$V = W^{1,1}(\R^n)$, $H = L^2(\R^n)$
 	...
 	injectivity follows from density of $V$ in $H$. $V \subseteq H $
\end{example}
